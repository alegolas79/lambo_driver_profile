...
df <- data %>% 
  filter(test!="test")%>%
  select(avg_daily_max, overall_max, overall_mean, mean_day_length_r_g, perc_days) %>%
  mutate(mean_day_length_r_g = mean_day_length_r_g/3600)
ggpairs(df, 
  columnLabels = c("Average daily maximum speed", "Maximum speed", "Average speed", "Average time spent driving in a day", "Percentage of days car was used"))+
  theme_light()
```
...
```
##  Analisi piloti professionisti
```{r pairs_2, fig.height=15, fig.width=25, echo=FALSE, message=FALSE, warning=FALSE} 
library(GGally)
df <- data %>% 
  filter(test!="test")%>%
  select(avg_daily_max, overall_max, overall_mean, mean_day_length_r_g, perc_days) %>%
  mutate(mean_day_length_r_g = mean_day_length_r_g/3600)%>%
  mutate(pilota = ifelse(avg_daily_max >=200, "Fast", ifelse(avg_daily_max < 100, "Slow", "Average")))
ggpairs(df[-6], 
  columnLabels = c("Average daily maximum speed", "Maximum speed", "Average speed", "Average time spent driving in a day", "Percentage of days car was used"), aes(color =df$pilota))+
  theme_light()
```

```{r setup_types}
data <- data %>%
  mutate(user_type = ifelse(perc_days == 0, "Uso limitato", NA))
```