...
cluster <- read.csv("C:/Users/l.dorsa/Desktop/Lambo/cluster_green.csv")
...
#Funzione per ottenere medie e massimi di vari valori per individuare differenze nei cluster
...
#FUnzione per trovare la % di giorni in cui si è usata una determinata driving mode
...
vr %>%
  ggplot(aes(x=as.factor(cluster), y=((round(avg_daily_max_throttle, digits = 0)))))+
  geom_boxplot(aes(color = as.factor(cluster)), lwd = 1)+
  theme_light()+ 
  xlab("Cluster")+
  ylab("Maximum throttle")+
  stat_summary(geom="text", fun.y=quantile,
               aes(label=sprintf("%1.1f", ..y..), color=factor(cluster)),
...
vr %>%
  ...
  stat_summary(geom="text", fun.y=quantile,
               aes(label=sprintf("%1.1f", ..y..), color=factor(cluster)),
               position=position_nudge(x=0.52), size=5)+
  theme(axis.text.x = element_text(face="bold", 
                                   size=12),
        axis.text.y = element_text(face="bold",
                                   size=12),
...
dm2 <- dm2 %>%
  mutate(dm1 = ifelse(perc_dm_1==0, "No", "Yes"),
         dm2 = ifelse(perc_dm_2==0, "No", "Yes"),
         dm3 = ifelse(perc_dm_3==0, "No", "Yes"))

a <- dm2 %>%
  ggplot(aes(x = cluster, fill = as.factor(dm1)))+
  geom_bar(position = "fill")+
  xlab("")+
  ylab("")+ 
  ggtitle("Charisma strada")+
  scale_fill_manual("Used driving mode", values = c("No" = "#C00000", "Yes" = "#007FC2"))+
  labs(fill = "Used driving mode")+
  theme_light()+
  ...
  theme(plot.background = element_rect(fill = "#EEEEEE"), legend.background =     element_rect(fill = "#EEEEEE"), legend.key = element_rect(fill = "#EEEEEE"))
b <- dm2 %>%
  ggplot(aes(x = cluster, fill = as.factor(dm2)))+
  geom_bar(position = "fill")+
  xlab("")+
  ylab("")+ 
  ggtitle("Charisma sport")+
...
c <- dm2 %>%
  ggplot(aes(x = cluster, fill = as.factor(dm3)))+
  geom_bar(position = "fill")+
  xlab("")+
  ylab("")+ 
  ggtitle("Charisma corsa")+
...
dm2 <- dm2 %>%
  select(vin, cluster, dm1, dm2, dm3)

dm3 <- dm2 %>%
  select(!vin)%>%
  group_by(cluster)%>%
  summarise(dm1 = sum(dm1=="Yes"), dm2 = sum(dm2=="Yes"), dm3 = sum(dm3=="Yes"))
dm3 <- dm3 %>%
  pivot_longer(cols = c(dm1, dm2, dm3), names_to = "driving_mode", values_to = "n")
dm3 <- dm3 %>%
  mutate(driving_mode = ifelse(driving_mode=="dm1", "Charisma strada",
                               ifelse(driving_mode=="dm2", "Charisma sport",
                                      "Charisma corsa")))
dm3$driving_mode2 <- factor(dm3$driving_mode, levels = c("Charisma strada", "Charisma sport", "Charisma corsa"))

dm3 %>%
  ggplot(aes(x= fct_reorder(driving_mode,n), y=n, fill=as.factor(cluster)))+
  ...
  theme(axis.text.y = element_text(face="bold",
        ...
        plot.title = element_text(face="bold",size=14))+
  theme(plot.background = element_rect(fill = "#EEEEEE"), legend.background =     element_rect(fill = "#EEEEEE"), legend.key = element_rect(fill = "#EEEEEE"))+
  geom_text(aes(label = n), vjust = -0.2,  position = position_dodge(.9))
